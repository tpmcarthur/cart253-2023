...
let circle = {
    ...
    y: 0,
    size: 100,
    trail: [] // creating an empty trail array as a property of the circle
}
...
function draw() {
    ...
    // Use a for loop to go through each element in the circles trail array in order
    for (let i = 0; i < circle.trail.length; i++) {
        // Get element at the index indicated by i (0, then 1, then 2 etc.)
        let element = circle.trail[i];
        // drae an ellipse the same size as the circle at that position 
